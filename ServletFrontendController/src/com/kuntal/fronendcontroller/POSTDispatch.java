 ...
 *
 */
public class POSTDispatch implements IDispatch {

	private static final Logger LOGGER = Logger.getLogger(POSTDispatch.class.getName());
	...
	private static POSTDispatch dispatch;
	
	/**
	 * Singleton design pattern using static block
	 */
	static {
		dispatch = new POSTDispatch();
	}
	
	public static POSTDispatch getPOSTDispatcher() {
		return dispatch;
	}
		...
		if (duplicateMethodNames.size() <= 7) {
			...
			for (String method : duplicateMethodNames) {
				if (!method.equalsIgnoreCase("wait") && !method.equalsIgnoreCase("equals")
						...
						response.setStatus(404);
						throw new DuplicateMethodNameException(method+" - method is duplicated, please check controller's calss methods");
				} 
			}
		}
	}	
